...
async def create_zone(camera_id: uuid.UUID, zone_in: ZoneCreate, db: AsyncSession = Depends(get_db), user: User = Depends(require_role(Role.SUPERVISOR))):
...
async def list_zones(camera_id: uuid.UUID, db: AsyncSession = Depends(get_db), user: User = Depends(require_role(Role.VIEWER))):
    result = await db.execute(select(ZoneModel).where(ZoneModel.camera_id == camera_id))
    return result.scalars().all()


@router.put("/zones/{zone_id}", response_model=ZoneResponse)
async def update_zone(zone_id: uuid.UUID, zone_in: ZoneUpdate, db: AsyncSession = Depends(get_db), user: User = Depends(require_role(Role.SUPERVISOR))):
...
async def delete_zone(zone_id: uuid.UUID, db: AsyncSession = Depends(get_db), user: User = Depends(require_role(Role.SUPERVISOR))):
    zone = await db.get(ZoneModel, zone_id)
    if not zone:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Zone not found")
